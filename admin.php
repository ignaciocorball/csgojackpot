...
if(isset($_GET['sendcomission'])){
    if($_GET['sendcomission']=='2'){
        $userid='76561198042582486';
        $token='obGu0S0X';
    }else{
        $userid='76561198042582486';
        $token='obGu0S0X';
    }
	if( $totalcomissionitems <= 0 ) {
            header("Location: admin.php?error");		
			die("Not enough Items to send");
		}
    ...
    $string=trim($string,'/');
    if(!$mysql->query('INSERT INTO `queue` (`gameid`,`userid`,`token`,`items`,`status`) VALUES ("0","'.$userid.'","'.$token.'","'.$string.'","active")')) echo $mysql->error;
	if(!$mysql->query('UPDATE `houseitems` SET `ininventory`="0" WHERE `id`="'.$citem['id'].'"')) echo $mysql->error;
	

	header("Location: admin.php?sucess&worth=$totalcomission&items=$totalcomissionitems");
...
?>
	<body style="overflow:auto;"><div id="content2" style="overflow:auto;">
     <div class="title"> <center> BOT CONSOLE LOG </center></div>

	 <?php 
	 if(isset($_GET["sucess"])) {
	  $variable = $_GET["items"];
	  $variable2 = $_GET["worth"];
      echo "<p class='test12341234'>Sucess! $variable Items has been sent - Worth ~ $$variable2</p>";
      
   
   }; 
   
   	 if(isset($_GET["error"])) {
      echo "<p class='mycss123123'>Error! Not enough items to send</p> <br>";
     ...
     <center><h1>Total comissions so far (in bot's inventory)</h1></center>
	 <center><h1><b><br>~ $<?=myround($totalcomission)?></b> (<?=$totalcomissionitems?> items) <br> <br></h1></center>
	 

     <br/><br/>
	 <center>
	 	 <form action="transfercash.php" method="post" style="display: -webkit-inline-box;">	
	
        <center><a href="admin.php?sendcomission" class="myButton">Send skins to admin account</a> <br>
		<br></center>
...
<div class="title"> <center>ROUND HISTORY</center></div>
        ...
        while($round=$roundq->fetch_assoc()){
            if($round['totalvalue']>0){
                ...
                if(preg_match('#sent#',$offer['status'])){
                    echo' <br><b style="background-color:darkgreen;color:white;">'.$offer['status'].'</b>';
                }elseif(preg_match('#active#',$offer['status'])){
                    echo'<br><b style="background-color:orange;color:black;">'.$offer['status'].'</b>';
                }else{
                    echo'<br><b style="background-color:darkred;color:white;">'.$offer['status'].'</b>';
                }
                ...
                echo' <br><tr>';
                    echo'<td><br> <center><img src="'.$winnerinfo['avatar'].'" alt="." style="height:120px;width:120px"/> </center></td>';
                    echo'<td>';
                    echo'<table>';
                    echo'<tr> <br><td><b><a href="http://steamcommunity.com/profiles/'.$round['winneruserid'].'" target="_blank">'.htmlspecialchars($winnerinfo['name']).'</a></b> ('.$round['winneruserid'].')</td></tr>';
                ...
                if($sentitemsnum!=$shouldvesent){
                    echo'<br><span style="background-color:darkred;color:white;">Error? <b>'.$sentitemsnum.'</b> items sent in the offer. The round had <b>'.$itemsnum.'</b> items.<br/><br/><br/>';
                }else{
                    echo'<br><span style="background-color:darkgreen;color:white;">All is good! <b>'.$sentitemsnum.'</b> items sent in the offer. The round had <b>'.$itemsnum.'</b> items.<br/><br/><br/>';
                }
                echo'</td></tr>';
