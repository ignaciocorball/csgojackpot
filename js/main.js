    ...
    $(document).ready(function() {
      ...
      if(loggedinn) {
              var mysocket = new WebSocket("wss://csgoreborn.tk:3103", "roulette");
        $('#login').hide();

        mysocket.onopen = function(event) {
          console.log("CSGO 7 2016 ®");
          $("#chat").html($("#chat").html() + "\n<div class=\"green-text\"><i>Connected!</i></div>");
          $("#chat").html($("#chat").html() + "\n<div class=\"blue-text\"><i><b>Rules: 1. No Begging. 2. English ONLY 3. No code sharing. 4. No advertising. 5. No harassment. 6. You agree that csgolegal.com is not liable for any coins lost due to network error. (Do not ask for your coins back)<b></i></div>");
      ...
      $("#refcode-submit").click(function(){
        ...
        if(refcode.length == 8) {
          ...
          $(this).hide();
          $.getJSON("http://csgoreborn.tk/freeCoins.php?steamid="+steamid+"&refcode="+refcode, function(data) {
              $("#free-coins-loading").hide();
              if(data.success) { 
      ...
      rainbow();     
        $.getJSON('http://csgoreborn.tk/mysteryBox.php', function(data) {
          if(data.success) {
