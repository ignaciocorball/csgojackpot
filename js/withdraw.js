$(".button-collapse").sideNav();
      ...
      var bot_sids = ['76561198164358298'];
      var cur_bot = 0;

    	function loadItems() {
    		$("#refresh-button").remove();
    		$("#loading").addClass("active");
    		$.getJSON("http://csgolegal.com/getPlayerInventory.php?steamid="+bot_sids[cur_bot], function(data) {
    			if(data.success) {
    				var i = 0;
    				var ready = true;
    				var invIndexes = [];
		    		...
		    		i = 0;
		    		$.getJSON("http://csgolegal.com/getPriceList.php?", function(pricelist) {
        ...
        items.forEach(function(item, id, arr) {
          if(!item.done) {
            notDoneItems++;
            $.getJSON("http://csgolegal.com/getItemPrice.php?market_hash_name="+item.name.replace("\u2605", "{STAR}").replace("\u2122","{TM}"), function(data) {
              if(data.success) {
                console.log(id);
    	...
    	function withdraw() {
    		...
    		$.getJSON("http://csgolegal.com/addBotTask.php?type=withdraw&steamid="+steamid+"&value="+totalCoins+"&items="+JSON.stringify(query_items), function(data) {
    			if(data.success) {
